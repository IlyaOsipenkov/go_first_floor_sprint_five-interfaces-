...
func (t Training) TrainingInfo() InfoMessage {
	return InfoMessage{
		t.TrainingType, t.Duration, t.distance(), t.meanSpeed(), t.Calories(),
	}
}
...
func (r Running) Calories() float64 {
	durationInH := r.Duration.Hours()
	//first step - i have no idea how to do right naming in this situation. Is "Steps" ok?
	firstStep := (CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight
	//second step
	secondStep := firstStep / MInKm * durationInH
	//third step
	thirdStep := secondStep * MinInHours
...
func (w Walking) Calories() float64 {
	durationInH := w.Duration.Hours()
	//first step
	firstStep := (math.Pow(w.meanSpeed(), 2) / w.Height) * CaloriesSpeedHeightMultiplier * w.Weight
	//second step
	secondStep := CaloriesWeightMultiplier*w.Weight + firstStep
	//third step
	thirdStep := secondStep * durationInH * MinInHours
...
func (s Swimming) Calories() float64 {
	...
	durationInH := s.Duration.Hours()
	//first step
	firstStep := s.meanSpeed() + SwimmingCaloriesMeanSpeedShift
	//second step
	secondStep := firstStep * SwimmingCaloriesWeightMultiplier * s.Weight * durationInH
